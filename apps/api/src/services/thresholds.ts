...
export async function evaluateThresholdsAndCreateAlertIfNeeded(vital: any) {
  const patient = await prisma.patient.findUnique({ where: { id: vital.patientId }, include: { branch: true } });
  if (!patient) return null;

  const profiles = await prisma.thresholdProfile.findMany({ where: { branchId: patient.branchId, type: vital.type } });
